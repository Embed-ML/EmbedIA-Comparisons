  ...
  for (i=0; i<output->height; i++){
   ...
   /* Serial.print(i);
    ...
    Serial.println(suma,5);*/
      suma = suma + ((dfixed)filter.bias<<CORRIMIENTO);
      suma=suma>>CORRIMIENTO;
      if (suma > FIX_MAX)
          suma = FIX_MAX;
      else if (suma < -FIX_MAX)
              suma = -FIX_MAX;
...
fixed neuron_forward(neuron_t neuron, flatten_data_t input){
  ...
  result =  result + ((dfixed)neuron.bias<<CORRIMIENTO);
  result=result>>CORRIMIENTO;
