	...
	for (i=0; i<output->height; i++){
		for (j=0; j<output->width; j++){
			...
			for (c=0; c<filter.channels; c++){
			 ...
			 //suma += FLOAT_TO_FIXED(FIXED_TO_FLOAT(filter.weights[(c*filter.kernel_size*filter.kernel_size)+k*filter.kernel_size+l]) * FIXED_TO_FLOAT(input.data[(c*input.height*input.width)+(i+k)*input.width+(j+l)]));

		 /*    Serial.print(FIXED_TO_FLOAT(filter.weights[(c*filter.kernel_size*filter.kernel_size)+k*filter.kernel_size+l]),5);
			 Serial.print(" x ");
			 Serial.print(FIXED_TO_FLOAT(input.data[(c*input.height*input.width)+(i+k)*input.width+(j+l)]),5);
			 Serial.print(" = ");
			 Serial.println(FIXED_TO_FLOAT( fixed_mul(filter.weights[(c*filter.kernel_size*filter.kernel_size)+k*filter.kernel_size+l] , input.data[(c*input.height*input.width)+(i+k)*input.width+(j+l)])),5);*/
...
void conv2d_layer(conv_layer_t layer, data_t input, data_t * output){
  ...
  //output->data     = (fixed*)malloc(sizeof(fixed)*output->channels*output->height*output->width);
...
void dense_forward(dense_layer_t dense_layer, flatten_data_t input, flatten_data_t * output){
  ...
  //output->data = (fixed*)malloc(sizeof(fixed)*dense_layer.n_neurons);
...
void max_pooling_2d(uint16_t strides, data_t input, data_t* output){
  ...
  //output->height = ceil(((float)input.height-strides+1)/strides);
  //output->width = ceil(((float)input.width-strides+1)/strides);
  ...
  /*for(c=0; c<input.channels; c++){
  	for(i=0; i<input.height-1; i+=strides){
  		for (j=0; j<input.width-1; j+=strides){
  			for(aux1=0; (aux1<strides)&&(aux1+i<input.height); aux1++){
  				for(aux2=0; (aux2<strides)&&(aux2+j<input.width); aux2++){
  					aux = input.data[c*input.height*input.width+(i+aux1)*input.width+(j+aux2)];
  					if (aux > max){
  						max = aux;
  					}
  				}
  			}
  			output->data[cant] = max;
  			cant++;
  			max = -255;
  		}
  	}
...
void flatten_layer(data_t input, flatten_data_t * output){
  ...
  //output->data = (fixed*)malloc(sizeof(fixed)*output->length);
...
void print_flatten_data_t(flatten_data_t data){
	...
	for(i=0;i<data.length;i++){
		Serial.print(FIXED_TO_FLOAT(data.data[i]),5);
		Serial.print(" ");
	}
	Serial.println();
	Serial.println("----------------");
	...
	for(c=0;c<filtro.channels;c++){
		for(h=0;h<filtro.kernel_size;h++){
			for(w=0;w<filtro.kernel_size;w++){
				Serial.print(FIXED_TO_FLOAT(filtro.weights[h*filtro.kernel_size+w]),5);
				Serial.print(" ");
			}
			Serial.println();
		}
