...
int main()
{
    ...
    FILE * outFile = fopen("FROMPC_EmbediaFloat_numerosyletras-all.csv","w");
    ...
    while (fgets(line, 10000, stream) && j<10000) //Leo lineas hasta que no hayan mas
    {
        j++;
        ...
        input.data[0] = atof(res);
        //printf("Input data -> %s\n",res);
        for(int i=1;i<TAM_ENTRADA;i++){
            res = strtok(NULL,", ]");
            input.data[i]=atof(res);
        }
        //Realizo la predicción
        uint8_t prediccion = model_predict(input,&resultados);
        fprintf(outFile,"%2d ; %2d ; %2d ; N/A ; %2.2f",j,atoi(expectedOutput),prediccion,resultados.data[prediccion]);
        printf("%2d ; %2d ; %2d ; N/A ; %2.2f",j,atoi(expectedOutput),prediccion,resultados.data[prediccion]);
