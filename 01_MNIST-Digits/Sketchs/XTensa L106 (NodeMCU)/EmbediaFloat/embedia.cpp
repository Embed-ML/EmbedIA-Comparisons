	...
	for(c=0;c<matriz.channels;c++){
		for(h=0;h<matriz.height;h++){
			for(w=0;w<matriz.width;w++){
		        ...
		        Serial.print( matriz.data[c*matriz.height*matriz.width+h*matriz.width+w], 5 );
				Serial.print(" ");
			}
			Serial.println();
		}
	...
	for(c=0;c<filtro.channels;c++){
		for(h=0;h<filtro.kernel_size;h++){
			for(w=0;w<filtro.kernel_size;w++){
				Serial.print((filtro.weights[h*filtro.kernel_size+w]),5);
				Serial.print(" ");
			}
			Serial.println();
		}
